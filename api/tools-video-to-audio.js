...
function getAudioArgs(format, inputPath, outputPath) {
  if (format === "mp3") {
    return [
      "-y",
      "-i", inputPath,
...
module.exports = async function handler(req, res) {
  ...
  const memberId = safeSegment(body.memberId || req.headers["x-nf-member-id"] || "anonymous");
  ...
  if (!AUDIO_FORMATS.has(audioFormat)) {
    return json(res, 400, {
      ok: false,
      error: "Invalid audioFormat",
      allowed: Array.from(AUDIO_FORMATS)
    });
  }
  ...
  const inputExt = (safeOriginal.split(".").pop() || "mp4").toLowerCase();
  const baseName = safeOriginal.replace(/\.[^/.]+$/, "") || "video";

  const tempDir = await fsp.mkdtemp(path.join(os.tmpdir(), "nf-audio-"));
  ...
  try {
    ...
    await runCommand(ffmpegPath, getAudioArgs(audioFormat, inputFile, outputFile));

    const outputPath = `tools/video-to-audio/${memberId}/${jobId}/audio.${audioFormat}`;
    ...
    if (uploadRes.error) {
      throw new Error(uploadRes.error.message || "Failed to upload audio");
    }
    ...
    console.error("tools-video-to-audio error:", err);
    return json(res, 500, {
      ok: false,
