...
async function smGetProject(projectId) {
  const r = await fetch(
    `${SUBMAGIC_BASE}/projects/${encodeURIComponent(projectId)}`,
    {
  ...
  if (!r.ok) {
    throw new Error(j?.message || `SUBMAGIC_${r.status}`);
  }
  return j;
}
...
module.exports = async function handler(req, res) {
  ...
  if (!dbId) {
    return res.status(400).json({ ok: false, error: "MISSING_DB_ID" });
  }
  ...
  try {
    ...
    // Important: return 200 so Submagic doesn't retry forever
    return res.status(200).json({ ok: true });
  }
};
